...
Eventuous connectors allow subscribing to events from EventStoreDB (source) and produce derived events to another system (sink). Some sinks also support reduce-like operations that work as [projections][1].
...
operations in parallel using the [partitioning filter][2].
The only partition key supported right now is the **stream name**. In addition, each sink is fully instrumented for
...
The _direct sink_ mode allows sending events to an endpoint directly, allowing to do anything with those events. For example, you can use the generic gRPC sink and create a custom projector to any database using any language that supports gRPC. The current implementation requires the sink to maintain the checkpoint, unlike the other modes, because the direct sink has no access to any persistent storage.

### Producer mode

In _producer_ mode, the connector subscribes to the source and produces events to the sink. What events will
look like on the other side depends on the sink implementation.

The producer mode is most useful for re-publishing domain events to a broker, or to be used as
an archive or backup. For example, the Elasticsearch sink can be used in combination with
the [archive event store][3] to keep archived events in a cheaper
...
The _projector_ mode allows implementing projections that are built using other stacks than .NET. Basically,
the projector mode implements the [MapReduce][4] pattern. Using a
...
[1]: https://eventuous.dev/docs/read-models/rm-concept
[2]: https://eventuous.dev/docs/subscriptions/pipes/#partitioning-filter
[3]: https://eventuous.dev/docs/persistence/event-store#event-store-with-archive
